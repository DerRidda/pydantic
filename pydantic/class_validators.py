...
def _generic_validator_cls(validator: AnyCallable, sig: 'Signature', args: Set[str]) -> 'ValidatorCallable':
    ...
    elif args == {'config', 'loc'}:
        return lambda cls, v, values, field, config, loc: validator(cls, v, config=config, loc=loc)
    elif args == {'values', 'config', 'field'}:
        return lambda cls, v, values, field, config, loc: validator(cls, v, values=values, config=config, field=field)
    elif args == {'values', 'loc', 'field'}:
        return lambda cls, v, values, field, config, loc: validator(cls, v, values=values, loc=loc, field=field)
    elif args == {'values', 'config', 'loc'}:
        return lambda cls, v, values, field, config, loc: validator(cls, v, values=values, config=config, loc=loc)
    elif args == {'config', 'loc', 'field'}:
        return lambda cls, v, values, field, config, loc: validator(cls, v, config=config, loc=loc, field=field)
    else:
        # args == {'values', 'config', 'loc', 'field'}
        return lambda cls, v, values, field, config, loc: validator(
            cls, v, values=values, config=config, loc=loc, field=field
        )
...
def _generic_validator_basic(validator: AnyCallable, sig: 'Signature', args: Set[str]) -> 'ValidatorCallable':
    ...
    if has_kwargs:
        ...
        return lambda cls, v, values, field, config, loc: validator(v, values=values, config=config, field=field)
        ...
        # args == {'loc', 'values', 'config', 'field'}
        return lambda cls, v, values, field, config, loc: validator(
            v, loc=loc, values=values, config=config, field=field
        )
